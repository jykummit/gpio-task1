...
#define PG_SIZE 8	//Page size to read and write into eeprom
#define SLAVE_ADDRESS (0x50)	//eeprom i2c address
#define SLAVE_DEVICE_NAME "eeprom_slave" //slave device name
#define MEM_SIZE 256  //eeprom buffer size
...
static struct i2c_adapter* i2c_adapter;  // i2c adapter structure - bus related info
static struct i2c_client* eeprom_client;  //i2c client structure - device connected to bus info
static char* kernel_buffer;  //kernel buffer	  
static char ebuf_loc = '0';  //eeprom loction
...
static ssize_t eeprom_read(struct file* file,char* __user buf,size_t len,loff_t* off){
	...
	// Reading Page Wise
	if(ebuf_loc%PG_SIZE != 0){
		memset(page_buffer,0x00,PG_SIZE);

		read_len = ((ebuf_loc/PG_SIZE) +1)* PG_SIZE - ebuf_loc;
...
static ssize_t eeprom_write(struct file* file,const char* __user buf,size_t len,loff_t* off){
	...
	if(ebuf_loc%PG_SIZE != 0){
		...
		write_len = ((ebuf_loc/PG_SIZE) +1)* PG_SIZE - ebuf_loc;
